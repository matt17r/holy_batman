...
namespace :quotes do
  desc 'Create final deduplicated master quote list with all manual resolutions applied'
  task create_final_master: :environment do
    puts "\n" + "=" * 80
    puts "CREATING FINAL MASTER QUOTE LIST (DEDUPLICATED)"
    puts "=" * 80
    ...
    # Manual resolution map - maps variations to preferred form
    resolution_map = {
    ...
    # Quotes to completely exclude (duplicates)
    exclude_quotes = [
      'Holy Jack In The Box - 2' # This is a duplicate of "Holy Jack in the Box"
    ]

    # Apply resolutions to both lists
    resolved_buzzfeed = buzzfeed_quotes.map { |q| resolution_map[q] || q }.compact
    resolved_fandom = fandom_quotes
      .reject { |q| exclude_quotes.include?(q) } # Remove excluded quotes
